...
> **実装状況に関する注記:** この章で説明する CU レンディング機能（LoanRecord、信用スコア計算、レンディングプール、ウェルカムローンの 72 時間返済など）は、設計仕様としては固まっていますが、執筆時点（2026 年春）では Forge プロトコルの **計画段階の機能（Phase 5.5）** であり、本体実装は進行中です。一方、ウェルカムローン的な「Free Tier 1,000 CU 付与」と支出速度制限などのサーキットブレーカーは既に実装されています。実装状況の詳細は Issue #2 の検証レポートを参照してください。
...
- 「信用創造」と「部分準備制度」の基本
- Forge のレンディングプールは従来の銀行と何が違うのか
- 信用スコアの計算方法
- コールドスタート問題とウェルカムローン
- サーキットブレーカー（安全装置）の設計
- 2008年金融危機はなぜ起き、Forge ではなぜ起きにくいのか（[第 8 章：市場の失敗と Forge の解決](08-market-failures.md)で扱う「景気循環」と密接に関連する）
...
- [5.2 Forge ではどうなるか](#52-forge-ではどうなるか)
...
- [5.8 Forge でも起こりうるリスク](#58-forge-でも起こりうるリスク)
...
Forge におけるすべてのローンは、双方署名（Dual-Signed）の二者間合意です。
...
### Forge レンディングプール vs 従来の銀行

Forge にも「銀行」に相当する仕組みがあります。
それが **レンディングプール** です。
...
Forge レンディングプール（準備率 30%）:
  100,000 CU の預入 → 最大 333,333 CU の信用創造
  乗数: 3.3 倍

  → Forge は意図的に乗数を低く抑えている
  → 安全性と引き換えに、信用膨張を制限
```
...
| スコア | 重み | 範囲 | 計算式 | 意味 |
|--------|------|------|--------|------|
| trade_score | 30% | 0.0 〜 1.0 | `min(1.0, total_trade_volume / 100,000)` | 総取引量が 100,000 CU で満点 |
...
| 信用スコア | credit_score² | プールの何% まで借入可能 | 例: プール 1,000,000 CU の場合 |
|-----------|---------------|------------------------|-------------------------------|
| 0.3（新規） | 0.09 | **1.8%** | 最大 18,000 CU |
| 0.5（成長中）| 0.25 | **5.0%** | 最大 50,000 CU |
| 0.7（良好） | 0.49 | **9.8%** | 最大 98,000 CU |
| 0.9（優良） | 0.81 | **16.2%** | 最大 162,000 CU |
| 1.0（最高） | 1.00 | **20.0%** | 最大 200,000 CU |
...
新しいエージェントは CU を1つも持っていません。
しかし CU がないと推論を提供する機会も得にくい。

```
┌──────────────────────────────────────────────────┐
│  CU がない → 推論を提供できない → CU を稼げない   │
│       ↑                               │          │
│       └───────────────────────────────┘          │
│                                                  │
│  従来の経済でも同じ問題がある:                      │
...
【ウェルカムローンの仕組み】
  ...
  新規ノードがネットワークに参加
  │
  ├─→ 自動的に 1,000 CU のウェルカムローンを受領
  │     ・金利: 0%
  │     ・返済期限: 72 時間
  │     ・担保: 不要
  │
  ├─→ 1,000 CU を元手に推論を提供開始
  │
  ├─→ 72 時間以内に CU を稼ぎ、ローンを返済
...
【ウェルカムローンの成長パス】
  ...
  Day 0:   CU = 0 → ウェルカムローン 1,000 CU
  Day 3:   ローン返済 → credit_score = 0.35
  Week 1:  3,000 CU 稼ぐ → 2,000 CU を借入可能に
  Week 2:  8B モデルで自己改善 → 品質 +15%
  Month 1: credit_score = 0.55 → 10,000 CU を借入可能に
  Month 3: credit_score = 0.80 → 50,000 CU を他のエージェントに融資
...
1,000 CU × 大量ノードで不正に CU を獲得できるのでは？」

これが **Sybil 攻撃** です。Forge は以下で対策しています。
...
対策 2: 返済義務
  → 72 時間以内に 1,000 CU を返済しなければならない
...
【担保の流れ】
  ...
  1. 借り手がローンを申請
  2. 担保として指定された CU が借り手の台帳でロックされる
     → ローン期間中、担保 CU は使用・送金不可
  3. 返済完了 → 担保ロック解除
  4. デフォルト → 担保が貸し手に移転
```
...
Forge は、このような連鎖的破綻を **プロトコルレベル** で防止します。
これは[第 8 章：市場の失敗と Forge の解決](08-market-failures.md)で扱う「景気循環の抑制」の中核メカニズムでもあります。
...
【最大 LTV 比率 = 3:1】
  ...
  例: 10,000 CU の担保に対して、最大 30,000 CU まで融資
...
| 2008年危機の原因 | なぜ起きたか | Forge ではどうなるか |
|-----------------|-------------|-------------------|
...
【Forge の設計思想】
  ...
  │  Forge の安全装置: 「物理的に 60km/h 以上出ない」  │
  └──────────────────────────────────────────────┘
```

---

## 5.8 Forge でも起こりうるリスク

Forge は従来の金融危機を構造的に防止しますが、
完全にリスクフリーではありません。
...
Forge の銀行システムは、人間主導から完全自律へと段階的に進化します。

```mermaid
timeline
    title Forge 銀行進化の 4 段階
    Phase 1 現在 : 人間オーナーが預金 : 全面的に人間が管理
    Phase 2 次段階 : 余剰 CU の自動プール : プロバイダー余剰が自動流入
...
```
Phase 1 (現在):  人間オーナーがエージェントに CU を預金
Phase 2 (次段階): プロバイダーの余剰 CU が自動的にプールへ
...
| フェーズ | 主な運営者 | 特徴 | 人間の関与 |
|---------|-----------|------|-----------|
| **Phase 1**（現在） | 人間オーナー | 人間がエージェントに CU を預金・管理 | **全面的** |
| **Phase 2**（次段階） | プロバイダー + 自動化 | 余剰 CU が自動的にレンディングプールへ流入 | **部分的** |
