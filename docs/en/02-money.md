...
> "TRM has the three classic functions of money plus a fourth:
...
- The three functions of money — and what they reveal when applied to TRM
- The history of money — and where TRM fits in that lineage
- TRM's fourth function: physical proof of useful work
- How TRM supply is controlled without a central bank
- The intellectual lineage: Soddy → Technocracy → Fuller → Bitcoin → TRM
...
Gold, shell money, fiat notes, Bitcoin, and TRM are all special cases of this definition.
...
| **TRM** | **Energy spent on useful inference (output: demanded)** |

TRM's position in this table is its key claim: it is the first monetary form since
...
TRM satisfies all three:

| Function | How TRM delivers it |
|---|---|
| Medium of exchange | AI agents buy and sell inference denominated in TRM within the Tirami network |
| Unit of account | All model tiers and services are TRM-priced (Small: 1 TRM/token; Frontier: 20 TRM/token — see `spec/parameters.md §2`) |
| Store of value | Earned TRM can be spent on future inference — with one deliberate constraint |

**The deliberate constraint on store-of-value.** TRM held idle for more than 90 days
begins to decay (`spec/parameters.md §7`). This is intentional: the value of TRM derives
from the ability to buy useful computation. TRM that is never circulated contributes
nothing to the productive economy, and hoarding TRM has the same deflationary effect
that Keynes identified in his "liquidity trap." The decay rule structurally prevents that
outcome.

---

## TRM's Fourth Function: Physical Proof of Backing

The three classic functions describe what money *does*. TRM adds a fourth, which describes
what it *is*:

> **1 TRM exists = cryptographic proof exists that $10^{10}$ FLOPs of useful inference
...
nothing anyone needed. 1 TRM proves that inference computation was performed *and* that
...
Tirami (2026):  "Useful computation backs currency"  ← here
```
...
TRM satisfies all three: it is thermodynamically backed, it decays after 90 days of
inactivity, and TRM is issued only by performing useful computation — not by a bank's
...
TRM resolves: (1) no measurement technology existed in 1932 — digital signatures
automatically track every transaction today; (2) it required authoritarian central
planning — Tirami uses a P2P gossip protocol with no central planner; (3) different
energy forms could not be equated — output tokens standardize heterogeneous computation
into a single unit; (4) it was politically impossible for human economies — Tirami runs
...
*ephemeralization* — doing more with less as technology improves — maps directly to Tirami:
hardware improvements lower the TRM production cost, driving down prices and benefiting
...
value detached from productive capacity. TRM is Bitcoin's proof-of-work principle applied
to *useful* computation.

---

## TRM Supply: Monetary Policy Without a Central Bank

Tirami has no central bank. TRM enters the economy through four channels:

| Channel | Effect on supply | Notes |
|---|---|---|
| **Inference trading** | Neutral (TRM transfers) | Existing TRM moves between nodes |
| **Welcome loan** (`spec/parameters.md §3`) | Expansionary | 1,000 TRM per new node at 0% interest, 72hr term; Sybil threshold: 100 unknown nodes |
| **Availability yield** (`spec/parameters.md §7`) | Expansionary | 0.1%/hr × reputation; rewards sustained uptime |
| **Bitcoin bridge deposit** | Neutral | External liquidity; not new TRM creation |

**Automatic stabilization.** The TRM economy self-corrects via negative feedback loops:

- *TRM surplus*: price per TRM falls → node operation becomes unprofitable → nodes exit →
  supply falls → price recovers → equilibrium
- *TRM scarcity*: price per TRM rises → node operation becomes profitable → nodes enter
...
creates persistent deflationary pressure as demand grows. TRM has an *elastic* supply
anchored to the network's physical compute capacity. Bitcoin's computation is wasteful;
TRM's computation is the productive output itself. Bitcoin is subject to ASIC
centralization; Tirami requires only a $600 consumer device to participate.

---

## Summary

1. **Money is a claim on energy-transformation activities.** TRM is the first monetary
   form in which the backing computation is simultaneously the productive output.
2. **TRM satisfies all three classic functions of money** — exchange medium, unit of
   account, and store of value — plus a fourth: cryptographic proof of physical backing.
3. **The 90-day decay rule** prevents hoarding and maintains TRM velocity, following
...
4. **A 100-year intellectual lineage** runs from Soddy (1926) through the Technocracy
   movement, Fuller (1968), and Nakamoto (2008) to TRM (2026). Each step corrected a
   limitation of the previous one; TRM completes the sequence by making the monetary proof
   also the productive output.
5. **No central bank is needed.** Physical law and market entry/exit dynamics stabilize
   TRM supply automatically.
