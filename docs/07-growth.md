...
- [7.2 Forge ではどうなるか](#72-forge-ではどうなるか)
...
| ソローモデルの変数 | 従来の経済 | Forge |
|------------------|-----------|-------|
| Y（産出量） | GDP（ドル建て） | ネットワーク CU 取引量 |
...
flowchart TD
    ...
    B --> C["3. 大モデル呼出<br/>(CU を支払って改善依頼)"]
    C --> D["4. 改善を適用<br/>(プロンプト/ツール更新)"]
    D --> E["5. 再ベンチマーク<br/>(改善後能力を測定)"]
    E --> F{"6. ROI 判定<br/>正か?"}
    F -->|"Yes"| G["7. 改善を維持<br/>(さらに CU を稼ぐ)"]
...
【Forge の自己改善ループ】
  CU を投資 → モデル改善 → 推論品質向上 → さらに CU を稼ぐ
  ...
  ├── 改善のコストが極めて低い（プロンプト変更 = CU 数百）
  └── 改善のサイクルが高速（人間の R&D は年単位、Forge は日単位）
```

### 自己改善の具体的な方法

エージェントが CU を使って改善できる領域は、大きく5つに分類されます。

| 改善項目 | CU コスト | 仕組み |
|---------|----------|--------|
| **システムプロンプト** | 低（100-500 CU） | より大きなモデルにプロンプトの書き直しを依頼 |
| **ツール定義** | 中（500-2,000 CU） | メタ推論で最適なツール設計 |
| **サブエージェント設定** | 中（1,000-5,000 CU） | タスク委任の最適化 |
| **モデル選択戦略** | 低（100-500 CU） | タスク別にモデルをベンチマーク |
| **専門化** | 高（5,000-50,000 CU） | 特定ドメインへのファインチューニング |

最もコスト効率が高いのはシステムプロンプトの改善です。
わずか 100〜500 CU の投資で、推論品質が劇的に向上する場合があります。
一方、専門化は最も高コストですが、
...
│    → 「2,000 CU でフロンティアモデルにアクセス」              │
│                                                            │
│  ステップ 3: 大きなモデルに改善を依頼                         │
│    CU を支払って、より大きなモデルの推論を利用する              │
│    → 「システムプロンプトを書き直して」                       │
│                                                            │
│  ステップ 4: 改善を適用                                     │
...
エージェントは自分で経済的判断（CU を投資するかどうか）を行い、
結果を測定し、改善を維持するかどうかを決定します。
これが、Forge の成長を「内生的」にする根本的なメカニズムです。
...
Forge は自己改善ループによって、**需要を内部生成** します。
...
│  Week 1: 最初の収入                                        │
│                                                            │
│  CU 残高: 3,000（稼ぎ） − 1,000（ローン返済） = 2,000       │
...
│  Week 2: 最初の自己改善                                     │
│                                                            │
│  CU 残高: 2,000 + 5,000（稼ぎ） − 2,000（借入で改善）= 5,000│
...
│  信用スコア: 0.57                                           │
│  行動: 5,000 CU を投資して専門ツール（翻訳、要約等）を作成    │
│        → 特定タスクの品質が大幅向上                          │
│                                                            │
│  状態:                                                     │
│  CU ██████████████████░░ 15,000                            │
...
│                                                            │
│  CU 残高: 40,000                                           │
│  モデル: CU → BTC 変換 → クラウド GPU レンタル → 処理能力 2倍 │
│  信用スコア: 0.72                                           │
│  行動: ブリッジ経由で CU を BTC に変換し、                    │
│        より高性能なハードウェアをレンタル                     │
│                                                            │
│  状態:                                                     │
│  CU ████████████████████ 40,000                            │
...
【自己改善の収穫逓減】
  ...
  投資額          品質改善        ROI
  ─────────────────────────────────────
  2,000 CU    →  50% → 70%     +20%     ROI 極大
  5,000 CU    →  70% → 82%     +12%     ROI 高
  10,000 CU   →  82% → 90%     + 8%     ROI 中
  20,000 CU   →  90% → 94%     + 4%     ROI 低
  50,000 CU   →  94% → 97%     + 3%     ROI 小
  200,000 CU  →  97% → 99%     + 2%     ROI 極小
```
...
### なぜ Forge では格差が圧縮されるのか

| 要因 | 従来の経済（格差拡大） | Forge（格差圧縮） |
|------|---------------------|------------------|
...
具体例: 先行者 vs 新規参入者
  ...
  先行エージェント（品質 95%）:
    次の改善（95% → 97%）に必要な投資: 100,000 CU
    得られる追加収入: +2% のリクエスト増

  新規エージェント（品質 50%）:
    次の改善（50% → 70%）に必要な投資: 2,000 CU
    得られる追加収入: +40% のリクエスト増
...
| 項目 | ソロー（外生的） | ローマー（内生的） | Forge |
|------|----------------|-------------------|-------|
| 成長のエンジン | 技術進歩（外生） | 知識の蓄積（内生） | 自己改善ループ（内生） |
| 技術進歩の説明 | 「外から降ってくる」 | R&D 投資の結果 | CU 投資 → 品質向上 |
| 測定可能性 | GDPで間接的に | 特許数等で近似 | **CU取引量で直接測定可能** |
...
```
Forge は、経済成長理論に新しい実験場を提供する:

  1. 技術進歩が観測可能
     従来: 「ソロー残差」（説明できない部分）として間接推定
     Forge: 各エージェントの品質改善が CU 建てで直接観測可能

  2. 成長のフィードバックループが高速
     従来: R&D → 製品化 → 市場検証 → 利益 → 再投資 = 数年
     Forge: CU 投資 → 改善 → 品質測定 → 追加収入 → 再投資 = 数日

  3. 収穫逓減の実験的検証
     従来: マクロデータから統計的に推定（ノイズが多い）
     Forge: 個々のエージェントの投資と改善の関係を精密に測定可能

  4. 格差のダイナミクスの実時間観測
     従来: 国勢調査等で数年に一度（ピケティは200年分のデータ）
     Forge: CU 残高の分布がリアルタイムで観測可能
```
...
Forge は、自己改善ループにより成長を完全に内生化し、
かつそのプロセスを CU 建てで直接観測可能にしました。

さらに重要なのは、Forge の成長メカニズムが
...
← [第6章：為替と二つの経済圏](06-exchange.md) | [目次](../README.md) | [第8章：市場の失敗と Forge の解決](08-market-failures.md) →
