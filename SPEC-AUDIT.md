# Forge Economics — 仕様書監査レポート

> 現在のリポジトリ状態と「あるべき姿」の差分監査
> 監査日: 2026-04-07

このドキュメントは、`forge-economics` リポジトリの現状と、宣言された仕様 (`spec/forge-economics-spec.md` v0.1) および README の主張との間にある乖離を網羅的に列挙します。各項目には対応する GitHub Issue 番号が紐付けられます。
...
| ID | 問題 | 詳細 |
|----|------|------|
| C-1 | **CU 定義の数値矛盾** | `README.md:21` は `1 CU = 1 億回の検証済み推論演算` と記述。一方 `spec/forge-economics-spec.md:57`、`docs/01-value.md`、`docs/02-money.md` 等すべての他ドキュメントは `1 CU = 10 億 FLOP` と記述。**100 倍の差**があり、CU 経済の根本数値が矛盾している。 |
...
| H-4 | **glossary に CU 定義の不整合を含むリスク** | `docs/appendix-glossary.md:71` は正しく `1 CU = 10 億 FLOP` だが、README の C-1 と整合させるための監査と対策が必要。 |
...
| ID | 問題 | 詳細 |
|----|------|------|
| M-1 | **chapter 13 が存在しない issue を参照** | `docs/13-open-questions.md:49` が `forge-economics GitHub リポジトリの Issues #4〜#8` を批判の出典として引用。しかし当該リポジトリの issue は本監査で初めて作成されるため、現時点で issue #4〜#8 は存在しない。引用の整合性が崩れている。 |
...
| M-6 | **welcome loan の数値が複数箇所で重複定義** | `1,000 CU, 0% 金利, 72 時間` が spec, glossary, chapter 5, chapter 9 などに散在。シングルソース化されていないため将来の変更時に矛盾を生むリスク。 |
...
| L-6 | **forge 本体への相互参照が薄い** | docs から `forge` リポジトリの該当 Rust コードへの直リンクがほぼない。「実装と理論の橋渡し」が弱い。 |
...
16. L-6: `forge` リポジトリへの相互参照リンクを各章に追加
...
| ID | Issue | タイトル | ラベル |
|----|-------|--------|-------|
| C-1 | [#24](https://github.com/clearclown/forge-economics/issues/24) | README の CU 定義 (1 億 → 10 億) を修正 | critical, documentation, consistency |
| C-2 | [#25](https://github.com/clearclown/forge-economics/issues/25) | `books/` フォルダを作成、リンク切れを修正 | critical, missing-content, infrastructure |
| C-3 | [#26](https://github.com/clearclown/forge-economics/issues/26) | `LICENSE` ファイル (MIT) を作成 | critical, infrastructure |
| H-1 | [#27](https://github.com/clearclown/forge-economics/issues/27) | spec を v0.2 に更新 (Chapter 11-14 を反映) | high, spec-update, consistency |
| H-2 | [#28](https://github.com/clearclown/forge-economics/issues/28) | glossary に Chapter 11-14 の新用語を追加 | high, documentation, consistency |
| H-3 | [#29](https://github.com/clearclown/forge-economics/issues/29) | bibliography を AI 経済学時代まで拡張 | high, documentation |
| H-5 | [#30](https://github.com/clearclown/forge-economics/issues/30) | CLAUDE.md のディレクトリ宣言を実態に合わせる | high, documentation, consistency |
| M-1 | [#31](https://github.com/clearclown/forge-economics/issues/31) | Chapter 13 の Issue #4-#8 引用を解決 | medium, consistency |
| M-3 | [#32](https://github.com/clearclown/forge-economics/issues/32) | 序章 (00-introduction) の章マップを更新 | medium, documentation, consistency |
| M-4 | [#33](https://github.com/clearclown/forge-economics/issues/33) | CHANGELOG.md を作成 | medium, infrastructure |
| M-5 | [#34](https://github.com/clearclown/forge-economics/issues/34) | CONTRIBUTING.md を作成 | medium, infrastructure |
| M-6 | [#35](https://github.com/clearclown/forge-economics/issues/35) | 数値定数をシングルソース化 (spec/parameters.md) | medium, spec-update |
| L-1 | [#36](https://github.com/clearclown/forge-economics/issues/36) | 英語版翻訳プロジェクトの立ち上げ | low, i18n, translation |
| L-2 | [#37](https://github.com/clearclown/forge-economics/issues/37) | CI による自動検証 (markdown lint, link check) | low, infrastructure |
| L-3 | [#38](https://github.com/clearclown/forge-economics/issues/38) | 章内 ToC を自動生成 | low, documentation |
| L-6 | [#39](https://github.com/clearclown/forge-economics/issues/39) | forge リポジトリへの相互参照を追加 | low, documentation |
